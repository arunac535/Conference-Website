...
const TABLE_HEAD = ["Authors/ Registration", "Members (in INR)", "Non- Members (in INR)", "Foreign Authors (in USD)"];
const data = [
  {
  ...
  return (
    <>
      ...
      <main className="pt-10 pb-20  bg-gray-200 ">
        ...
        <div className="w-screen min-h-screen container mx-auto flex flex-col items-center justify-center">
        ...
        <Card className="h-full w-500 mx-12 shadow-xl mt-20">
          <table className="w-full min-w-max table-auto text-left rounded-lg border">
            <thead>
              <tr>
                {TABLE_HEAD.map((head) => (
                  <th key={head} className="text-2xl font-bold bg-blue-gray-100 p-4 ">
                    <Typography
                      variant="h5"
                      color="blue-gray"
            ...
            <tbody>
              {data.map(({ category, members, nonmembers, foreignauthors,}) => (
